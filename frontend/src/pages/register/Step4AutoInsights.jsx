...
export default function Step4AutoInsights({ formData, updateFormData, nextStep, prevStep }) {
  ...
  if (!data) {
    return (
      <div className="text-white text-center">
        <p>No delivery data available. Please go back and fetch your data.</p>
      </div>
    );
  }
  ...
  const insights = {
    ...
    avgWeeklyEarnings: data.avgWeeklyEarnings,
    avgMonthlyEarnings: data.avgMonthlyEarnings,
    workingHours: data.workingHours,
    riskLevel: data.riskLevel,
    performance: data.rating >= 4.5 ? 'Excellent' : data.rating >= 4.0 ? 'Good' : 'Average'
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg">
            ...
            <div className="text-2xl font-bold text-blue-900">
              {formatCurrency(insights.avgDailyEarnings, 'INR')}
            </div>
            <div className="text-xs text-blue-700 mt-1">Per working day</div>
          </div>
          ...
          <div className="bg-gradient-to-br from-green-50 to-green-100 p-4 rounded-lg">
            ...
            <div className="text-2xl font-bold text-green-900">
              {formatCurrency(insights.avgWeeklyEarnings, 'INR')}
            </div>
            <div className="text-xs text-green-700 mt-1">6 days/week</div>
          </div>

          <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-4 rounded-lg">
            <div className="flex items-center gap-2 text-purple-600 mb-2">
              <DollarSign className="w-5 h-5" />
              <span className="text-sm font-medium">Monthly Avg</span>
            </div>
            <div className="text-2xl font-bold text-purple-900">
              {formatCurrency(insights.avgMonthlyEarnings, 'INR')}
            </div>
            <div className="text-xs text-purple-700 mt-1">~26 days/month</div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2 text-orange-600 mb-1">
                ...
                <span className="text-sm font-medium">Average Working Hours</span>
              </div>
              <div className="text-3xl font-bold text-orange-900">{insights.workingHours} hrs/day</div>
            </div>
            <div className="text-right">
              <div className="text-sm text-orange-700">Performance</div>
              <div className="text-lg font-bold text-orange-900">{insights.performance}</div>
              <div className="text-xs text-orange-600">⭐ {data.rating} rating</div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className={`flex items-center gap-2 mb-1 ${
                insights.riskLevel === 'low' ? 'text-green-600' : 'text-yellow-600'
              }`}>
                <MapPin className="w-5 h-5" />
                <span className="text-sm font-medium">Risk Assessment</span>
              </div>
              <div className={`text-2xl font-bold ${
                insights.riskLevel === 'low' ? 'text-green-900' : 'text-yellow-900'
              }`}>
            ...
            <div className="text-right">
              <div className={`text-sm ${
                insights.riskLevel === 'low' ? 'text-green-700' : 'text-yellow-700'
              }`}>
                Primary Zone
              </div>
